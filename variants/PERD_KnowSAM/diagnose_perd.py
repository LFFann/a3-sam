...
from perd_modules import foreground_boundary_tube, logits_to_hard_label, normalized_entropy_map
from prediction_perd import add_common_args
...
def parse_args():
    ...
    parser.set_defaults(split="val", save_dir="./Results/PERD_KnowSAM/fold_0/diagnosis_val")
...
def curve_means(perd_output):
    tube = perd_output.boundary_tube > 0.5
    orig_prob = torch.softmax(perd_output.orig_curve_logits, dim=2)[:, :, 1:2]
    atten_prob = torch.softmax(perd_output.atten_curve_logits, dim=2)[:, :, 1:2]
    orig_values = []
    atten_values = []
...
def main():
    ...
    with torch.no_grad():
        for index, sampled_batch in enumerate(loader):
            ...
            perd_output = trainer.perd_probe.probe(
                trainer.sam_model,
                image,
                image_embeddings,
                boxes_embedding,
            ...
            pred_label = logits_to_hard_label(perd_output.y0_logits).detach().cpu().numpy()[0]
            ...
            boundary_region = to_numpy_map(perd_output.boundary_tube) > 0.5
            error_map = np.logical_xor(pred_boundary, gt_boundary).astype(np.float32)
            reliable_map = (1.0 - error_map).astype(np.float32)

            trust = to_numpy_map(perd_output.trust_map)
            pc = to_numpy_map(perd_output.pc_map)
            ed = to_numpy_map(perd_output.ed_map)
            ji = to_numpy_map(perd_output.ji_map)
            valid = to_numpy_map(perd_output.valid_map)
            ...
            orig_curve, atten_curve = curve_means(perd_output)
            save_curve(curve_dir / f"{case_stem}_response_curve.png", trainer.perd_probe.delta_levels, orig_curve, atten_curve)

            mask = boundary_region
            reliable_flat = reliable_map[mask].reshape(-1)
            error_flat = error_map[mask].reshape(-1)
            scores = {
                "trust": trust[mask].reshape(-1),
                "pc_neg": (-pc)[mask].reshape(-1),
                "ed": ed[mask].reshape(-1),
            ...
            row = {
                ...
                "trust_mean": safe_mean_on_mask(trust, mask),
                "pc_mean": safe_mean_on_mask(pc, mask),
                "ed_mean": safe_mean_on_mask(ed, mask),
                ...
                "trust_reliable_auroc": binary_auc(reliable_flat, scores["trust"]),
                "trust_reliable_aupr": binary_aupr(reliable_flat, scores["trust"]),
                "ed_reliable_auroc": binary_auc(reliable_flat, scores["ed"]),
                "pc_reliable_auroc": binary_auc(reliable_flat, scores["pc_neg"]),
            }
            rows.append(row)
            logging.info(
                "case=%s boundary_error=%.6f trust=%.6f pc=%.6f ed=%.6f valid=%.6f",
                case_name,
    ...
    for key in ["trust", "pc_neg", "ed", "entropy_neg", "agreement", "edge"]:
