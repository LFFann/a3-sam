...
from ra_modules import ResponseAuditProbe
from prediction_ACDC import test_single_volume
from utils.losses_ra import DiceLoss, KDLoss, WeightedKDLoss, loss_diff1, loss_diff2
...
class Trainer(nn.Module):
    def __init__(self, args):
        ...
        self.ra_enabled = bool(int(getattr(args, "ra_enabled", getattr(args, "perd_enabled", 1))))
        self.response_audit_probe = ResponseAuditProbe(args)
    ...
    def train(self, volume_batch, label_batch, iter_num):
        ...
        ra_stats = {
            "ra_admission_mean": 1.0,
            "ra_pc_mean": 0.0,
            "ra_esr_mean": 0.0,
            "ra_ji_mean": 0.0,
            "ra_valid_ratio": 1.0,
            "ra_boundary_ratio": 0.0,
            "ra_saturated_ratio": 0.0,
            "ra_prompt_dominated_ratio": 0.0,
            "ra_prior_locked_ratio": 0.0,
            "ra_evidence_sensitive_ratio": 0.0,
            "ra_unidentifiable_ratio": 0.0,
            "ra_inside_change": 0.0,
            "ra_outside_change": 0.0,
            "ra_intervention_valid_ratio": 1.0,
        }
        ...
        if self.ra_enabled:
            _, boxes_embedding, _ = self.sam_model.super_prompt(image_embeddings)
            ra_output = self.response_audit_probe.probe(
                self.sam_model,
                volume_batch,
                image_embeddings,
                boxes_embedding,
            ...
            ra_stats.update(ra_output.stats)
            kd_weight = ra_output.kd_weight
            if getattr(self.args, "ra_baseline", getattr(self.args, "perd_baseline", "response_audit")) == "prompt_ensemble":
                kd_teacher = ra_output.orig_curve_logits.mean(dim=0).detach()
            else:
                kd_teacher = ra_output.y0_logits.detach()
        ...
        logging.info(
            ...
            "fusion_loss : %f UNet_lr_ : %10f ra_admission_mean : %f ra_pc_mean : %f "
            "ra_esr_mean : %f ra_ji_mean : %f ra_valid_ratio : %f ra_boundary_ratio : %f "
            "ra_saturated_ratio : %f ra_prompt_dominated_ratio : %f ra_prior_locked_ratio : %f "
            "ra_evidence_sensitive_ratio : %f ra_unidentifiable_ratio : %f",
            iter_num,
            sam_loss.item(),
            lr_,
            SGDL_loss.item(),
            ...
            fusion_loss.item(),
            UNet_lr_,
            ra_stats["ra_admission_mean"],
            ra_stats["ra_pc_mean"],
            ra_stats["ra_esr_mean"],
            ra_stats["ra_ji_mean"],
            ra_stats["ra_valid_ratio"],
            ra_stats["ra_boundary_ratio"],
            ra_stats["ra_saturated_ratio"],
            ra_stats["ra_prompt_dominated_ratio"],
            ra_stats["ra_prior_locked_ratio"],
            ra_stats["ra_evidence_sensitive_ratio"],
            ra_stats["ra_unidentifiable_ratio"],
        )
        return {
